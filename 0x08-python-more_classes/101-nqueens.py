...
Determines all possible solutions for placing N
...
where `r` and `c` represent the row and the column, respectively,
...
def solve_recursive(board, row, queens, solutions):
    ...
    for c in range(len(board)):
        if board[row][c] == " ":
            tmp_board = board_pcopys(board)
            tmp_board[row][c] = "Q"
            xout(tmp_board, row, c)
            solutions = solve_recursive(tmp_board, row + 1,
                                        queens + 1, solutions)
